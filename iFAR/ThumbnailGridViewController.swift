...
class ThumbnailGridViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        collectionView?.register(UINib(nibName: "ThumbnailGridCell", bundle: nil),forCellWithReuseIdentifier: "ThumbnailGridCell")
...
extension ThumbnailGridViewController {
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print("Selected IndexPath  section: \(indexPath.section)   row: \(indexPath.row) ")
     if let page = pdfDocument?.page(at: indexPath.item) {
            dismiss(animated: false, completion: nil)
